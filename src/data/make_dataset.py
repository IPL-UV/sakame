    ...
    def save_minicubes(self, data=None, window_size=5, save_name=None):
        ...
        for ivariable in self.variables:
            ...
            for itime in time_stamps:
                ...
                # Get the minicubes
                for ix, iy, ilat, ilon in window_xy(
                    data[ivariable].sel(time=itime), window_size=window_size
                ):
                    #             print('here')
                    itime_x.append(ix)
                    itime_y.append(iy)
...
class ToyData2D(object):
    def __init__(self, num_points=25, func="custom1", random_state=123):
        ...
        self.figure_path = (
            "/home/emmanuel/projects/2019_sakame/reports/figures/demos/gpr/"
        )
    ...
    def regress_f(self):
        ...
        if self.func == "custom1":
            ...
            x12_a = np.linspace(0, 20, self.num_points - round(self.num_points / 2))
            ...
            y = (
                np.hstack((self.k11 * self.x11, self.k12 * self.x12))
                + self.k2 * self.x2
            )
            ...
            x22_a = np.linspace(0, 20, self.num_points - round(self.num_points / 2))
    ...
    def plot_raw(self, demo=True):
        ...
        if demo:
            fig.suptitle("Regression: $Y=k_1 x_1 + k_2 x_2$", fontsize=20)
            ax.set_xlabel("X1")
            ax.set_ylabel("X2")
            ax.set_zlabel("Y")
            plt.show()
...
def get_class_data(
    num_points=250, num_training=0.7, noise=0.01, random_state=None, data_set="ellipse"
):
    x = dict()
    y = dict()

    # generate data from examples
    if data_set in ["ellipse", "ellipsoid"]:
        x["data"], y["data"] = make_circle_ellipse(
            num_points=num_points, noise=noise, verbose=None, seed=random_state
        )
        y["data"][y["data"] == 0] = -1

    elif data_set in ["moons", "Moons"]:
        x["data"], y["data"] = make_moons(
            n_samples=num_points, noise=noise, random_state=random_state
        )
        y["data"][y["data"] == 0] = -1

    elif data_set in ["circles", "Circles"]:
        x["data"], y["data"] = make_circles(
            n_samples=num_points, noise=noise, random_state=random_state, factor=0.5
        )
        y["data"][y["data"] == 0] = -1
    ...
    # scale the data
    x["data"] = StandardScaler().fit_transform(x["data"])

    # train/test split
    x["train"], x["test"], y["train"], y["test"] = train_test_split(
        x["data"], y["data"], train_size=num_training, random_state=random_state
    )
...
def get_xy_indices(spatial_resolution=7, return_coord=None):
    ...
    if spatial_resolution <= 1:
        ...
        raise ValueError(
            "Unrecognized spatial configuration: {}.".format(spatial_resolution)
        )
...
def window_xy(array, window_size=7):
    ...
    indices_vector, lat_coord, lon_coord = get_xy_indices(
        window_size, return_coord=True
    )
    ...
    for ilat in range(n_lat - window_size):
        for ilon in range(n_lon - window_size):
            ...
            if (
                np.sum(
                    np.isnan(data[ilat : ilat + window_size, ilon : ilon + window_size])
                )
                == 0
            ):

                x = data[ilat : ilat + window_size, ilon : ilon + window_size][
                    indices_vector == 1
                ]
                y = data[ilat : ilat + window_size, ilon : ilon + window_size][
                    lat_coord, lon_coord
                ]
